...
/**
 * Motore di simulazione "leggero" basato su GameAshtonTablut.
 * Ottimizzato per il Minimax: rimosso logging, gestione cache, ecc.
 * INCLUDE la logica di rilevamento pareggio, ma richiede
 * che la cronologia gli venga passata dall'esterno.
 */
public class SimulationEngine {

    // Lista delle cittadelle (regole fisse)
    private List<String> citadels;

    public SimulationEngine() {
        // Inizializza solo le regole delle cittadelle
    ...
    /**
     * Il metodo "cuore" della simulazione.
     * Controlla, applica la mossa, controlla le catture E controlla il pareggio.
     * Modifica l'oggetto State passato come parametro.
     *
     * @param state Lo stato (clonato) su cui applicare la mossa.
     * @param a L'azione da eseguire.
     * @param history Il Set di stati (come String) visti finora in questo percorso.
     * @return Lo stato modificato.
     */
    public State checkMove(State state, Action a, Set<String> history)
        ...
        // 1. VALIDAZIONE MOSSA
        // (Questa parte è identica a GameAshtonTablut, ma senza logging)
        ...
        // 2. APPLICAZIONE MOSSA
        state = this.movePawn(state, a);

        // 3. CONTROLLO CATTURE
        // (Se una cattura porta a una vittoria, il turno verrà
        // aggiornato di conseguenza all'interno di questi metodi)
        if (state.getTurn().equalsTurn("W")) { // Il turno è appena cambiato, quindi chi ha mosso era NERO
            state = this.checkCaptureBlack(state, a);
        } else if (state.getTurn().equalsTurn("B")) { // Chi ha mosso era BIANCO
            state = this.checkCaptureWhite(state, a);
        }
        
        // 4. CONTROLLO PAREGGIO E VITTORIA
        // Controlliamo dopo le catture, perché lo stato finale è quello che conta.
        
        // Se la mossa ha già portato a una vittoria (es. fuga del Re o cattura del Re),
        // non c'è bisogno di controllare il pareggio.
        if (state.getTurn().equals(State.Turn.WHITEWIN) || state.getTurn().equals(State.Turn.BLACKWIN)) {
            return state;
        }

        // --- ECCO LA LOGICA DI PAREGGIO ---
        // Controlla se il nuovo stato è già nella cronologia.
        if (history.contains(state.toString())) {
            state.setTurn(State.Turn.DRAW);
        }
        // --- FINE LOGICA PAREGGIO ---

        return state;
    }

    //
    // Da qui in poi ci sono tutti i metodi privati di checkCapture e movePawn
    // copiati da GameAshtonTablut e "alleggeriti" (rimosso logging e movesWithoutCapturing)
    //
    ...
    private State checkCaptureWhite(State state, Action a) {
        ...
        // controllo se ho vinto (fuga del Re)
